...
if not st.session_state.desired_outcome:
    ...
    with tab1:
        ...
        with col1:
            ...
            # Match index to prevent dropdown reset
        ...
        with col2:
            ...
            client_response = st.text_area("Client Response:", height=130)
            
            # Mapping Data
            st.caption("Map Coordinates (Whereabouts)")
            map_cols = st.columns(2)
            map_x = map_cols[0].slider("Left ⟷ Right", 0, 100, 50)
            map_y = map_cols[1].slider("Down ⟷ Up", 0, 100, 50)
            ...
            with log_col3:
                ...
                if st.button("Log Entry") and client_response:
                    st.session_state.history.append({
                        ...
                        "x_pos": map_x, "y_pos": map_y, "label": subject_x if subject_x else "Symbol"
                    })
                    st.rerun()
    ...
    with tab2:
        ...
        if metaphors:
            ...
            for i, item in enumerate(metaphors):
                with m_cols[i % 3]: st.success(f"🔮 **{item['label']}**\n\n{item['answer'][:100]}...")
        st.divider()
    ...
    with tab3:
        ...
        if not map_items:
            st.info("Tag symbols as 'Metaphor' and set their coordinates to see them here.")
        else:
            df = pd.DataFrame([{"x": i['x_pos'], "y": i['y_pos'], "Symbol": i['label']} for i in map_items])
            st.scatter_chart(df, x="x", y="y", color="Symbol", size=250)
            st.write("**Spatial Legend:**")
            st.table(df)
